...
def _reporter_lexico_gradient_boosting(system: LexicoGradientBoostingClassifier, X_test: pd.DataFrame) -> dict[str, Any]:
    """
    Reports the results of the fitted LexicoGradientBoostingClassifier algorithm on the test data.

    Args:
        system (LexicoGradientBoostingClassifier): The fitted LexicoGradientBoostingClassifier algorithm.
    ...
    if not system:
        raise ValueError("System's (LexicoGradientBoostingClassifier) not fitted yet.")
    return {
    ...
    def launch_experiment(self):
        experiment = ExperimentEngine(
            ...
            system_hyperparameters={
                "custom_system": LexicoGradientBoostingClassifier,
                "threshold_gain": 0.0015,
            },
            system_reporter=_reporter_lexico_gradient_boosting,
        )
