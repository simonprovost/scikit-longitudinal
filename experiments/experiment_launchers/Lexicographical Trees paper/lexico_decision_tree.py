...
import pandas as pd
from scikit_longitudinal.estimators.trees.lexicographical.lexico_decision_tree import LexicoDecisionTreeClassifier

from experiments.experiment_engine import ExperimentEngine


def _reporter_lexico_decision_tree(system: LexicoDecisionTreeClassifier, X_test: pd.DataFrame) -> dict[str, Any]:
    """
    Reports the results of the fitted LexicoDecisionTreeClassifier algorithm on the test data.

    Args:
        system (LexicoDecisionTreeClassifier): The fitted LexicoDecisionTreeClassifier algorithm.
    ...
    if not system:
        raise ValueError("System's (LexicoDecisionTreeClassifier) not fitted yet.")
    return {
        ...
        "best_pipeline": {
            ...
            "preprocessor": "None",
            "classifier": "LexicoDecisionTreeClassifier"
        },
    ...
    def launch_experiment(self):
        experiment = ExperimentEngine(
            ...
            system_hyperparameters={
                "custom_system": LexicoDecisionTreeClassifier,
                "threshold_gain": 0.0015,
                "class_weight": "balanced",
            },
            system_reporter=_reporter_lexico_decision_tree,
        )
