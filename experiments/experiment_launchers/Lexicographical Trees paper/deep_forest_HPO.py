...
from gama.utilities.longitudinal_config_space import CascadeForestClassifier


class CustomDeepForestClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, rf_params=None, random_state=None, max_layers=5):
        """
        Custom Deep Forest Classifier with two Random Forests and two Completely Random Forests.
        Allows hyperparameter tuning for the Random Forests.

        Parameters:
        - rf_params: dict, optional
            Parameters for the two main Random Forests.
        - random_state: int, optional
            Seed for reproducibility.
        - max_layers: int, optional
            Maximum layers in the CascadeForestClassifier.
        """
    ...
    def _initialize_estimators(self):
        # Two Random Forests with customizable hyperparameters
        rf1 = RandomForestClassifier(**self.rf_params, random_state=self.random_state)
        rf2 = RandomForestClassifier(**self.rf_params, random_state=self.random_state)
        # Two Completely Random Forests (max_features set to 1)
        crf1 = RandomForestClassifier(max_features=1, random_state=self.random_state)
        crf2 = RandomForestClassifier(max_features=1, random_state=self.random_state)
        return [rf1, rf2, crf1, crf2]

    def fit(self, X, y):
        """
        Fit the custom deep forest model on training data.
        ...
        """
        self._cascade = CascadeForestClassifier(random_state=self.random_state, max_layers=self.max_layers)
        self._cascade.set_estimator(self._initialize_estimators(), n_splits=2)
    ...
    def predict_proba(self, X):
        ...
        """
        return self._cascade.predict_proba(X)
...
def _load_dataset(
    ...
    Args:
        ...
        remove_target_waves (bool, optional): Whether to remove target waves from the dataset. Defaults to True.> To update the setup_data_parameters, to use one different from True.
        setup_features_group (str, optional): The method to set up the features group. Defaults to "elsa". > To update the setup_data_parameters, to use one different from "elsa".
...
def _reporter_deep_forest_gridsearch(system: GridSearchCV, X_test: pd.DataFrame, opt_metric: str) -> dict:
    """
    Reporter function for DeepForestClassifier with GridSearchCV.
    ...
    return {
        ...
        "best_pipeline": {
            ...
            "preprocessor": "None",
            "classifier": f"{sampler_name} & Lexic. DF with params {best_params}"
        },
...
class Launcher:
    ...
    def validate_parameters(self):
        ...
        if self.args.sampler not in ['vanilla', 'undersample', 'oversample', 'class_weight']:
            raise ValueError("Sampler must be one of: 'vanilla', 'undersample', 'oversample', or 'class_weight'.")
    ...
    def launch_experiment(self):
        ...
        sqrt_p = int(np.sqrt(self._num_features))

        # Define the sampler based on user input
        ...
        pipeline = Pipeline([
            ('sampler', sampler),
            ('classifier', CustomDeepForestClassifier(
                max_layers=5,
                random_state=self.args.random_state,
                rf_params={
                    "n_estimators": 300,  # Will be overwritten by GridSearchCV
                    "max_features": sqrt_p,  # Will be overwritten by GridSearchCV
                    "max_depth": 5,  # Will be overwritten by GridSearchCV
        ...
        param_grid = {
            'classifier__rf_params': [
                {
                    "n_estimators": 300,
                    "max_depth": md,
                    "max_features": mf,
                }
                for mf in [sqrt_p, int(1.5 * sqrt_p), int(2 * sqrt_p), int(np.log2(self._num_features))]
        ...
        experiment = ExperimentEngine(
            ...
            system_reporter=lambda system, X_test: _reporter_deep_forest_gridsearch(
                system,
                X_test,
...
def main():
    parser = argparse.ArgumentParser(description="Lexicographical Deep Forest with HPO and Class Imbalance Handling")
