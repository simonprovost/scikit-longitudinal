...
def _reporter_lexico_gradient_boosting_gridsearch(system: GridSearchCV, X_test: pd.DataFrame, opt_metric: str) -> dict[
    str, Any]:
    """
    Reporter function for LexicoGradientBoostingClassifier with GridSearchCV.
    ...
    return {
        ...
        "best_pipeline": {
            ...
            "preprocessor": "None",
            "classifier": f"{sampler_name} & Lexic. GB with params {best_params}"
        },
...
class Launcher:
    ...
    def launch_experiment(self):
        ...
        pipeline = Pipeline([
            ('sampler', sampler),
            ('classifier', LexicoGradientBoostingClassifier(
                learning_rate=0.1,  # Will be overwritten by grid search
                threshold_gain=0.0015,  # Will be overwritten by grid search
                random_state=self.args.random_state
            ))
        ])
        ...
        param_grid = {
            ...
            'classifier__threshold_gain': [0.01, 0.02, 0.03, 0.04]
        }
        ...
        experiment = ExperimentEngine(
            ...
            system_reporter=lambda system, X_test: _reporter_lexico_gradient_boosting_gridsearch(
                system,
                X_test,
...
def main():
    parser = argparse.ArgumentParser(
        description="Lexicographical Gradient Boosting with HPO and Class Imbalance Handling")
