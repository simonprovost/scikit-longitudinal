...
def _reporter_lexico_random_forest(
    system: LexicoRandomForestClassifier, X_test: pd.DataFrame
) -> dict[str, Any]:
    """
    Reports the results of the fitted LexicoRandomForestClassifier algorithm on the test data.

    Args:
        system (LexicoRandomForestClassifier): The fitted LexicoRandomForestClassifier algorithm.
    ...
    if not system:
        raise ValueError("System's (LexicoRandomForest) not fitted yet.")
    return {
...
class Launcher:
    ...
    def validate_parameters(self):
        ...
        if (
            not isinstance(self.args.n_outer_splits, int)
            or self.args.n_outer_splits <= 0
        ):
    ...
    def launch_experiment(self):
        experiment = ExperimentEngine(
            ...
            system_hyperparameters={
                "custom_system": LexicoRandomForestClassifier,
                "threshold_gain": 0.0015,
                "class_weight": "balanced",
            },
            system_reporter=_reporter_lexico_random_forest,
        )
