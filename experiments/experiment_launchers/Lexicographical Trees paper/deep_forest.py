...
def _reporter_deep_forest(system: CascadeForestClassifier, X_test: pd.DataFrame) -> dict[str, Any]:
    """
    Reports the results of the fitted CascadeForestClassifier algorithm on the test data.

    Args:
        system (CascadeForestClassifier): The fitted CascadeForestClassifier algorithm.
    ...
    if not system:
        raise ValueError("System's (DecisionTreeClassifier) not fitted yet.")
    return {
        "predictions": system.predict(X_test),
        "probability_predictions": system.predict_proba(X_test),
        "best_pipeline": {
            "data_preparation": "MerWavTimeMinus",
            "preprocessor": "None",
            "classifier": "DeepForestClassifier"
        },
    ...
    def launch_experiment(self):
        experiment = ExperimentEngine(
            ...
            system_hyperparameters={
                "custom_system": CascadeForestClassifier,
                "max_layers": 5,
            },
            system_reporter=_reporter_deep_forest,
        )
