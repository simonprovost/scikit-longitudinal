...
import pandas as pd
from scikit_longitudinal.estimators.ensemble.nested_trees.nested_trees import NestedTreesClassifier

from experiments.experiment_engine import ExperimentEngine


def _reporter_nested_trees(system: NestedTreesClassifier, X_test: pd.DataFrame) -> dict[str, Any]:
    """
    Reports the results of the fitted NestedTreesClassifier algorithm on the test data.

    Args:
        system (NestedTreesClassifier): The fitted NestedTreesClassifier algorithm.
    ...
    if not system:
        raise ValueError("System's (NestedTreesClassifier) not fitted yet.")
    return {
    ...
    def launch_experiment(self):
        experiment = ExperimentEngine(
            ...
            system_hyperparameters={
                "custom_system": NestedTreesClassifier,
                "inner_estimator_hyperparameters": {
                   "class_weight": "balanced",
                }
            },
            system_reporter=_reporter_nested_trees,
        )
