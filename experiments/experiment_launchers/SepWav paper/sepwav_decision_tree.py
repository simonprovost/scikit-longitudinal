...
def _reporter_sepwav(system: Union[Pipeline, SepWav], X_test: pd.DataFrame) -> dict:
    """
    Reporter function for SepWav with stacking using Decision Tree.
    ...
    return {
        ...
        "best_pipeline": {
            ...
            "preprocessor": "None",
            "classifier": f"{sampler_name} & SepWav Random Forest with stacking Decision Tree",
        },
...
class Launcher:
    ...
    def launch_experiment(self):
        ...
        stacking_meta_learner = DecisionTreeClassifier(class_weight='balanced' if self.args.sampler == 'class_weight' else None)
...
def main():
    parser = argparse.ArgumentParser(description="SepWav with Stacking Decision Tree and Class Imbalance Handling")
