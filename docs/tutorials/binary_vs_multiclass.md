...
!!! important "Dataset Used in Tutorials"
    Generate `extended_stroke_longitudinal.csv` once using the snippet in the [tutorials overview](overview.md), then reuse it here.

!!! tip "Prerequisite Reading"
    Read the [Temporal Dependency Guide](temporal_dependency.md) and the [first estimator tutorial](sklong_explore_your_first_estimator.md) first if you are new to `Sklong`.
