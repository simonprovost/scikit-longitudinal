...
# our dataset. We fit a :class:`~sklearn_fork.linear_model.Ridge` and check the
...
# We can use a :class:`~sklearn_fork.gaussian_process.kernels.ExpSineSquared` kernel
# which allows recovering the periodicity. The class
# :class:`~sklearn_fork.kernel_ridge.KernelRidge` will accept such a kernel.
#
...
#
# Now, we will use a
# :class:`~sklearn_fork.gaussian_process.GaussianProcessRegressor` to fit the same
...
# :class:`~sklearn_fork.gaussian_process.kernels.WhiteKernel` that is used to
