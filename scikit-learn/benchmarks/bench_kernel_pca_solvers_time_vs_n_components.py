...
# generate the grid
n_compo_range = [
    np.round(np.exp((x / (n_compo_grid_size - 1)) * np.log(max_n_compo))) for x in range(0, n_compo_grid_size)
]
...
# -----------------------
n_features = 2
X, y = make_circles(n_samples=(n_train + n_test), factor=0.3, noise=0.05, random_state=0)
...
ax.set_title(
    "kPCA Execution time comparison on %i samples with %i features, according to the choice of `eigen_solver`"
    % (n_train, n_features)
)
