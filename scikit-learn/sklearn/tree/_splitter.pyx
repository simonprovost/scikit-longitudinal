...
#          Provost Simon <sgp28@kent.ac.uk>
...
cdef class Splitter(BaseSplitter):
    ...
    def __cinit__(self, Criterion criterion, SIZE_t max_features,
                  ...
                  object random_state, float threshold_gain, list features_group):
        """
        Parameters
        ----------
        criterion : Criterion
...
cdef inline int node_lexicoRF_split(
    ...
    -----------
    splitter: Splitter
        The splitter object containing the necessary data and methods for the node splitting
    partitioner: Partitioner
        The partitioner object responsible for sorting and partitioning the samples
    criterion: Criterion
        The criterion object responsible for impurity calculations and split evaluations
    impurity: double
        The impurity of the current node
    split: SplitRecord*
        Pointer to a SplitRecord object to store the best split information
    n_constant_features: SIZE_t*
        Pointer to a SIZE_t variable to store the number of constant features
    threshold_gain: float
        Threshold value for considering information gain significantly better
    features_group: list
        List of lists representing the structure of the features in the dataset, such as [[smoke_w1, smoke_w2], [chol_w1, chol_w2]]

    Returns:
    --------
    int
        -1 in case of failure to allocate memory (and raise MemoryError) or 0 otherwise
    """
    # Find the best split
    cdef SIZE_t start = splitter.start
    cdef SIZE_t end = splitter.end
