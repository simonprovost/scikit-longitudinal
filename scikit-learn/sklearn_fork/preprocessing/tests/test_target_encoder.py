...
from sklearn_fork.model_selection import KFold, ShuffleSplit, StratifiedKFold, cross_val_score, train_test_split
from sklearn_fork.pipeline import make_pipeline
from sklearn_fork.preprocessing import KBinsDiscretizer, LabelEncoder, TargetEncoder
...
def test_encoding(categories, unknown_value, global_random_seed, smooth, target_type):
    ...
    for train_idx, test_idx in cv.split(X_train_array, y_train):
        ...
        expected_X_fit_transform[test_idx, 0] = cur_encodings[X_train_array[test_idx, 0]]

    target_encoder = TargetEncoder(smooth=smooth, categories=categories, cv=3, random_state=0)
    ...
    y_mean = np.mean(y_int)
    expected_encodings = _encode_target(X_train_array[:, 0], y_int, n_categories, smooth)
