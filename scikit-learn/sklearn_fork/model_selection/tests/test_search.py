...
from sklearn_fork.base import BaseEstimator, ClassifierMixin, is_classifier
from sklearn_fork.cluster import KMeans
from sklearn_fork.datasets import make_blobs, make_classification, make_multilabel_classification
from sklearn_fork.ensemble import HistGradientBoostingClassifier
from sklearn_fork.impute import SimpleImputer
from sklearn_fork.linear_model import LinearRegression, Ridge, SGDClassifier
...
from sklearn_fork.neighbors import KernelDensity, KNeighborsClassifier, LocalOutlierFactor
...
def test_custom_run_search():
    ...
    for attr in dir(gscv):
        if (
            attr[0].islower()
            and attr[-1:] == "_"
            and attr not in {"cv_results_", "best_estimator_", "refit_time_", "classes_"}
        ):
...
def test_callable_multimetric_clf_all_fits_fail():
    ...
    error_message = re.compile(
        f"All the 15 fits failed.+your model is misconfigured.+{individual_fit_error_message}",
        flags=re.DOTALL,
    )
