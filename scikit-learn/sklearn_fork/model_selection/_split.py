"""
The :mod:`sklearn_fork.model_selection._split` module includes classes and
...
class _BaseKFold(BaseCrossValidator, metaclass=ABCMeta):
    ...
    def split(self, X, y=None, groups=None):
        ...
        if self.n_splits > n_samples:
            raise ValueError(
                ("Cannot have number of splits n_splits={0} greater than the number of samples: n_samples={1}.").format(
...
class StratifiedGroupKFold(_BaseKFold):
    ...
    The difference between :class:`~sklearn_fork.model_selection.GroupKFold`
    and :class:`~sklearn_fork.model_selection.StratifiedGroupKFold` is that
    ...
    if not hasattr(cv, "split") or isinstance(cv, str):
        if not isinstance(cv, Iterable) or isinstance(cv, str):
            raise ValueError(
                ...
                "object (from sklearn_fork.model_selection) "
