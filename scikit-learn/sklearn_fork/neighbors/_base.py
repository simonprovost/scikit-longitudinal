...
from ..metrics._pairwise_distances_reduction import ArgKmin, RadiusNeighbors
...
def _check_precomputed(X):
    ...
    if graph.format not in ("csr", "csc", "coo", "lil"):
        raise TypeError(
            "Sparse matrix in {!r} format is not supported due to its handling of explicit zeros".format(graph.format)
        )
    ...
    if warn_when_not_sorted:
        warnings.warn(
            (
                ...
                " function sklearn_fork.neighbors.sort_graph_by_row_values to sort the"
                " input by row values, with warn_when_not_sorted=False to remove this"
                " warning."
            ),
            EfficiencyWarning,
        )
