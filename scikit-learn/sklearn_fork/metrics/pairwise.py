...
from ..utils import check_array, gen_batches, gen_even_slices, get_chunk_n_rows, is_scalar_nan
from ..utils._mask import _get_mask
from ..utils._param_validation import Hidden, Interval, MissingValues, Real, StrOptions, validate_params
...
def euclidean_distances(X, Y=None, *, Y_norm_squared=None, squared=False, X_norm_squared=None):
    ...
    if X_norm_squared is not None:
        ...
        if X_norm_squared.shape != (X.shape[0], 1):
            raise ValueError(
                f"Incompatible dimensions for X of shape {X.shape} and X_norm_squared of shape {original_shape}."
            )
    ...
    if Y_norm_squared is not None:
        ...
        if Y_norm_squared.shape != (1, Y.shape[0]):
            raise ValueError(
                f"Incompatible dimensions for Y of shape {Y.shape} and Y_norm_squared of shape {original_shape}."
            )
...
@validate_params({"X": ["array-like", "sparse matrix"], "Y": ["array-like", "sparse matrix", None]})
...
# Paired distances
@validate_params({"X": ["array-like", "sparse matrix"], "Y": ["array-like", "sparse matrix"]})
...
def additive_chi2_kernel(X, Y=None):
    ...
    chi2_kernel : The exponentiated version of the kernel, which is usually
        preferable.
    sklearn_fork.kernel_approximation.AdditiveChi2Sampler : A Fourier approximation
        to this kernel.
...
def pairwise_distances_chunked(
    ...
    **kwds : optional keyword parameters
    ...
    >>> import numpy as np
    >>> from sklearn_fork.metrics import pairwise_distances_chunked
...
def pairwise_distances(X, Y=None, metric="euclidean", *, n_jobs=None, force_all_finite=True, **kwds):
    ...
    scikit-learn, see :func:`sklearn_fork.metrics.pairwise.distance_metrics`
    function.
...
def kernel_metrics():
    ...
    The valid distance metrics, and the function they map to, are:
      ...
      metric            Function
      ===============   ========================================
      'additive_chi2'   sklearn_fork.pairwise.additive_chi2_kernel
      'chi2'            sklearn_fork.pairwise.chi2_kernel
      'linear'          sklearn_fork.pairwise.linear_kernel
      'poly'            sklearn_fork.pairwise.polynomial_kernel
      'polynomial'      sklearn_fork.pairwise.polynomial_kernel
      'rbf'             sklearn_fork.pairwise.rbf_kernel
      'laplacian'       sklearn_fork.pairwise.laplacian_kernel
      'sigmoid'         sklearn_fork.pairwise.sigmoid_kernel
      'cosine'          sklearn_fork.pairwise.cosine_similarity
      ===============   ========================================
...
def pairwise_kernels(X, Y=None, metric="linear", *, filter_params=False, n_jobs=None, **kwds):
    ...
    metric : str or callable, default="linear"
        ...
        :mod:`sklearn_fork.metrics.pairwise` are not allowed, as they operate on
