...
Gaussian Mixtures (:class:`~sklearn_fork.mixture.GaussianMixture`), and
neighbor-based approaches such as the kernel density estimate
(:class:`~sklearn_fork.neighbors.KernelDensity`).
...
:class:`~sklearn_fork.neighbors.KernelDensity` estimator, which uses the
...
:class:`~sklearn_fork.neighbors.KernelDensity` implements several common kernel
...
metrics (see :class:`~sklearn_fork.metrics.DistanceMetric` for a list of
