...
i.e. a model equivalent to :class:`~sklearn_fork.linear_model.LogisticRegression`
which is fitted via SGD instead of being fitted by one of the other solvers
in :class:`~sklearn_fork.linear_model.LogisticRegression`. Similarly,
`SGDRegressor(loss='squared_error', penalty='l2')` and
:class:`~sklearn_fork.linear_model.Ridge` solve the same optimization problem, via
different means.
...
The class :class:`sklearn_fork.linear_model.SGDOneClassSVM` implements an online
linear version of the One-Class SVM using a stochastic gradient descent.
Combined with kernel approximation techniques,
:class:`sklearn_fork.linear_model.SGDOneClassSVM` can be used to approximate the
solution of a kernelized One-Class SVM, implemented in
:class:`sklearn_fork.svm.OneClassSVM`, with a linear complexity in the number of
samples. Note that the complexity of a kernelized One-Class SVM is at best
quadratic in the number of samples.
:class:`sklearn_fork.linear_model.SGDOneClassSVM` is thus well suited for datasets
...
=====================
  ...
  * Finding a reasonable regularization term :math:`\alpha` is
    ...
    :class:`~sklearn_fork.model_selection.RandomizedSearchCV`, usually in the
