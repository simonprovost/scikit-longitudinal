...
.. currentmodule:: sklearn_fork.kernel_ridge
...
vector regression (:class:`~sklearn_fork.svm.SVR`). However, different loss
...
regularization. In contrast to :class:`~sklearn_fork.svm.SVR`, fitting
...
thus slower than :class:`~sklearn_fork.svm.SVR`, which learns a sparse model for
:math:`\epsilon > 0`, at prediction-time.

The following figure compares :class:`KernelRidge` and
:class:`~sklearn_fork.svm.SVR` on an artificial dataset, which consists of a
sinusoidal target function and strong noise added to every fifth datapoint.
The learned model of :class:`KernelRidge` and :class:`~sklearn_fork.svm.SVR` is
...
faster than fitting :class:`~sklearn_fork.svm.SVR` (both with grid-search).
However, prediction of 100000 target values is more than three times faster
with :class:`~sklearn_fork.svm.SVR` since it has learned a sparse model using only
...
:class:`KernelRidge` and :class:`~sklearn_fork.svm.SVR` for different sizes of the
training set. Fitting :class:`KernelRidge` is faster than
:class:`~sklearn_fork.svm.SVR` for medium-sized training sets (less than 1000
samples); however, for larger training sets :class:`~sklearn_fork.svm.SVR` scales
better. With regard to prediction time, :class:`~sklearn_fork.svm.SVR` is faster
...
the :class:`~sklearn_fork.svm.SVR`; :math:`\epsilon = 0` would correspond to a
dense model.
