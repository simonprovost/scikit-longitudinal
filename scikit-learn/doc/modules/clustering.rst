...
unlabeled data can be performed with the module :mod:`sklearn_fork.cluster`.
...
.. topic:: Input data
    ...
    These can be obtained from the classes in the :mod:`sklearn_fork.feature_extraction`
    ...
    in the :mod:`sklearn_fork.metrics.pairwise` module.
...
clustering algorithms, see :func:`sklearn_fork.cluster.kmeans_plusplus` for details
and example usage.
...
using :func:`sklearn_fork.neighbors.kneighbors_graph` to restrict
merging to nearest neighbors as in :ref:`this example
<sphx_glr_auto_examples_cluster_plot_agglomerative_clustering.py>`, or
using :func:`sklearn_fork.feature_extraction.image.grid_to_graph` to
...
.. warning:: **Connectivity constraints with single, average and complete linkage**
    ...
    :func:`sklearn_fork.neighbors.kneighbors_graph`. In the limit of a small
...
.. topic:: Memory consumption for large sample sizes
    ...
    - A sparse radius neighborhood graph (where missing entries are presumed to
      ...
      :meth:`sklearn_fork.neighbors.NearestNeighbors.radius_neighbors_graph`.
...
.. currentmodule:: sklearn_fork.metrics
...
The Fowlkes-Mallows index (:func:`sklearn_fork.metrics.fowlkes_mallows_score`) can be
...
(:func:`sklearn_fork.metrics.silhouette_score`)
...
(:func:`sklearn_fork.metrics.calinski_harabasz_score`) - also known as the Variance
...
(:func:`sklearn_fork.metrics.davies_bouldin_score`) can be used to evaluate the
...
cluster analysis as follows:
  ...
  >>> X = iris.data
  >>> from sklearn_fork.cluster import KMeans
  >>> from sklearn_fork.metrics import davies_bouldin_score
...
Contingency matrix (:func:`sklearn_fork.metrics.cluster.contingency_matrix`)
...
(:func:`sklearn_fork.metrics.cluster.pair_confusion_matrix`) is a 2x2
similarity matrix
