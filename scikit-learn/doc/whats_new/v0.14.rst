...
- Added :ref:`biclustering <biclustering>` algorithms
  (:class:`sklearn_fork.cluster.bicluster.SpectralCoclustering` and
  :class:`sklearn_fork.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`sklearn_fork.datasets.make_biclusters` and
  :func:`sklearn_fork.datasets.make_checkerboard`), and scoring metrics
  (:func:`sklearn_fork.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed :mod:`sklearn_fork.linear_model.stochastic_gradient.py` L2 regularization
...
- :class:`grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`sklearn_fork.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`sklearn_fork.decomposition.TruncatedSVD`
...
- The default number of components for
  :class:`sklearn_fork.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`sklearn_fork.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in :mod:`sklearn_fork.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- :mod:`sklearn_fork.ensemble.gradient_boosting` provides out-of-bag improvement
  :attr:`~sklearn_fork.ensemble.GradientBoostingRegressor.oob_improvement_`
...
- Fixed a bug in :class:`sklearn_fork.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in :class:`sklearn_fork.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`sklearn_fork.naive_bayes.MultinomialNB` and
  :class:`sklearn_fork.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Significant speed improvements for :class:`sklearn_fork.cluster.DBSCAN`
...
- Testing scikit-learn with ``sklearn_fork.test()`` is deprecated. Use
  ``nosetests sklearn_fork`` from the command line.
...
- :class:`sklearn_fork.neighbors.BallTree` has been refactored, and a
  :class:`sklearn_fork.neighbors.KDTree` has been
...
- :class:`sklearn_fork.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`sklearn_fork.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`sklearn_fork.linear_model.RidgeCV`.

- Sparse matrix support in :class:`sklearn_fork.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`sklearn_fork.ensemble.GradientBoostingRegressor`
  and :class:`sklearn_fork.ensemble.GradientBoostingClassifier`
...
- :class:`sklearn_fork.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`sklearn_fork.multiclass.OneVsRestClassifier` now has a
