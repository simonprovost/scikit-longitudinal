...
- |Efficiency| :func:`sklearn_fork.datasets.fetch_openml` now loads data by
...
- |Fix| add support for non-numeric data in
  :class:`sklearn_fork.impute.MissingIndicator` which was not supported while
  :class:`sklearn_fork.impute.SimpleImputer` was supporting this for some
...
- :mod:`sklearn_fork.neighbors` when ``metric=='jaccard'`` (bug fix)
...
- |Fix| Fixed :class:`sklearn_fork.neighbors.DistanceMetric` jaccard distance
...
- |API| Removed all mentions of ``sklearn_fork.externals.joblib``, and deprecated
  joblib methods exposed in ``sklearn_fork.utils``, except for
...
- |API| Deprecated :func:`sklearn_fork.datasets.fetch_mldata` to be removed in
...
- |Fix| Fixed a bug in :class:`sklearn_fork.linear_model.Lasso`
  where the coefficient had wrong shape when ``fit_intercept=False``.
  :issue:`10687` by :user:`Martin Hahn <martin-hahn>`.

- |Fix| Fixed a bug in :func:`sklearn_fork.linear_model.LogisticRegression` where the
...
- |Feature| The ``transform`` method of :class:`sklearn_fork.preprocessing.MultiLabelBinarizer`
...
- |Feature| An environment variable to use the site joblib instead of the
  ...
  is now exposed in :mod:`sklearn_fork.utils`.
...
- |Feature| A utility method :func:`sklearn_fork.show_versions()` was added to
...
- |Fix| Fixed a bug where calling :func:`sklearn_fork.base.clone` was not thread
...
- Checks for transformers now apply if the estimator implements
  ...
  :class:`sklearn_fork.base.TransformerMixin`. :issue:`10474` by `Joel Nothman`_.
