...
- Fix a bug where :class:`sklearn_fork.feature_selection.SelectFdr` did not
  exactly implement Benjamini-Hochberg procedure. It formerly may have
  selected fewer features than it should.
  :issue:`7490` by :user:`Peng Meng <mpjlu>`.

- :class:`sklearn_fork.manifold.LocallyLinearEmbedding` now correctly handles
...
- :class:`sklearn_fork.linear_model.LogisticRegressionCV` now correctly handles
  string labels. :issue:`5874` by `Raghav RV`_.

- Fixed a bug where :func:`sklearn_fork.model_selection.train_test_split` raised
  an error when ``stratify`` is a list of string labels. :issue:`7593` by
  `Raghav RV`_.

- Fixed a bug where :class:`sklearn_fork.model_selection.GridSearchCV` and
  :class:`sklearn_fork.model_selection.RandomizedSearchCV` were not pickleable
  because of a pickling bug in ``np.ma.MaskedArray``. :issue:`7594` by
  `Raghav RV`_.

- All cross-validation utilities in :mod:`sklearn_fork.model_selection` now
  ...
  The :class:`sklearn_fork.model_selection.GridSearchCV` will cross-validate each
...
- **The model_selection module**
  ...
  The new module :mod:`sklearn_fork.model_selection`, which groups together the
  functionalities of formerly :mod:`sklearn_fork.cross_validation`,
  :mod:`sklearn_fork.grid_search` and :mod:`sklearn_fork.learning_curve`, introduces new
...
- **Data-independent CV splitters enabling nested cross-validation**
  ...
  :mod:`sklearn_fork.model_selection`, are no longer initialized with any
...
- The cross-validation iterators are replaced by cross-validation splitters
  available from :mod:`sklearn_fork.model_selection`, allowing for nested
...
- Added support for substituting or disabling :class:`pipeline.Pipeline`
  ...
  interface that powers :mod:`sklearn_fork.grid_search`.
...
- Fix in :class:`sklearn_fork.model_selection.StratifiedShuffleSplit` to
...
- The :mod:`sklearn_fork.cross_validation`, :mod:`sklearn_fork.grid_search` and
  :mod:`sklearn_fork.learning_curve` have been deprecated and the classes and
  functions have been reorganized into the :mod:`sklearn_fork.model_selection`
...
sklearn_fork-ci, SmedbergM, srvanrell, Sébastien Lerique, Taranjeet, themrmax,
