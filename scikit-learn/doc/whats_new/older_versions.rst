...
- The old ``scikits.learn`` package has disappeared; all code should import
  from ``sklearn_fork`` instead, which was introduced in 0.9.
...
- Added :class:`~sklearn_fork.cross_validation.StratifiedShuffleSplit`, which is
  a :class:`~sklearn_fork.cross_validation.ShuffleSplit` with balanced splits,
  by Yannick Schwartz.

- :class:`~sklearn_fork.neighbors.NearestCentroid` classifier added, along with a
...
- Silhouette Coefficient cluster analysis evaluation metric added as
  :func:`~sklearn_fork.metrics.silhouette_score` by Robert Layton.
...
- Adjusted Mutual Information metric added as
  :func:`~sklearn_fork.metrics.adjusted_mutual_info_score` by Robert Layton.
...
- Improved documentation for developers and for the :mod:`sklearn_fork.utils`
  module, by `Jake Vanderplas`_.

- Vectorized 20newsgroups dataset loader
  (:func:`~sklearn_fork.datasets.fetch_20newsgroups_vectorized`) by
  `Mathieu Blondel`_.
...
- Make :func:`~sklearn_fork.preprocessing.scale` and
  :class:`~sklearn_fork.preprocessing.Scaler` work on sparse matrices by
  `Olivier Grisel`_
...
- :class:`~sklearn_fork.cross_validation.ShuffleSplit` can subsample the train
...
- The SVMlight dataset loader :func:`~sklearn_fork.datasets.load_svmlight_file` no
...
- The functions used for sparse coding, ``sparse_encode`` and
  ...
  :func:`~sklearn_fork.decomposition.sparse_encode`, and the shapes of the arrays
...
- Fixed an off-by-one error in the SVMlight/LibSVM file format handling;
  files generated using :func:`~sklearn_fork.datasets.dump_svmlight_file` should be
  re-generated. (They should continue to work, but accidentally had one
  extra column of zeros prepended.)

- ``BaseDictionaryLearning`` class replaced by ``SparseCodingMixin``.

- :func:`~sklearn_fork.utils.extmath.fast_svd` has been renamed
  :func:`~sklearn_fork.utils.extmath.randomized_svd` and the default
...
- The ``scikits.learn`` package was renamed ``sklearn_fork``. There is
  ...
  (make a backup first!)::
      ...
      find -name "*.py" | xargs sed -i 's/\bscikits.learn\b/sklearn_fork/g'
...
- The ``score_func`` argument of the
  ``sklearn_fork.cross_validation.cross_val_score`` function is now expected
...
- The ``sklearn_fork.hmm`` has been marked as orphaned: it will be removed
  from scikit-learn in version 0.11 unless someone steps up to
  contribute documentation, examples and fix lurking numerical
  stability issues.

- ``sklearn_fork.neighbors`` has been made into a submodule.  The two previously
...
- ``sklearn_fork.ball_tree.BallTree`` has been moved to
  ``sklearn_fork.neighbors.BallTree``.  Using the former will generate a warning.

- ``sklearn_fork.linear_model.LARS()`` and related classes (LassoLARS,
  LassoLARSCV, etc.) have been renamed to
  ``sklearn_fork.linear_model.Lars()``.

- All distance metrics and kernels in ``sklearn_fork.metrics.pairwise`` now have a Y
...
- ``sklearn_fork.metrics.pairwise.l1_distance`` is now called ``manhattan_distance``,
...
- Improved sparse matrix support, both in main classes
  ...
  sklearn_fork.svm.sparse and sklearn_fork.linear_model.sparse.
...
- New FastICA algorithm in module sklearn_fork.fastica
...
- Joblib is now a dependency of this package, although it is
  shipped with (sklearn_fork.externals.joblib).
