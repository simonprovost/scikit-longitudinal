...
- Improve narrative documentation and consistency in
  :mod:`sklearn_fork.metrics` for regression and classification metrics
  by `Arnaud Joly`_.

- Fixed a bug in :class:`sklearn_fork.svm.SVC` when using csr-matrices with
...
- The module ``sklearn_fork.linear_model.sparse`` is gone. Sparse matrix support
  was already integrated into the "regular" linear models.

- :func:`sklearn_fork.metrics.mean_square_error`, which incorrectly returned the
...
- Renamed function :func:`sklearn_fork.metrics.zero_one` to
  :func:`sklearn_fork.metrics.zero_one_loss`. Be aware that the default behavior
  in :func:`sklearn_fork.metrics.zero_one_loss` is different from
  :func:`sklearn_fork.metrics.zero_one`: ``normalize=False`` is changed to
  ``normalize=True``.
