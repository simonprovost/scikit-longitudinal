...
- ``nose`` is non-longer a runtime dependency to import ``sklearn_fork``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`sklearn_fork.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the :mod:`sklearn_fork.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the  :mod:`sklearn_fork.ensemble.gradient_boosting`
...
- Faster :class:`sklearn_fork.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- :mod:`sklearn_fork.hmm` is deprecated. Its removal is planned
