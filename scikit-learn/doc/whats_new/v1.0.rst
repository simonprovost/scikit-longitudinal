...
- |Fix| All :class:`sklearn_fork.metrics.DistanceMetric` subclasses now correctly support
...
- |Fix| All :class:`sklearn_fork.metrics.MinkowskiDistance` now accepts a weight
...
- |Fix| Compute `y_std` properly with multi-target in
  :class:`sklearn_fork.gaussian_process.GaussianProcessRegressor` allowing
...
- |Fix| Solve a bug in :func:`~sklearn_fork.utils.metaestimators.if_delegate_has_method`
...
- |Enhancement| :class:`~sklearn_fork.ensemble.HistGradientBoostingClassifier` and
  :class:`~sklearn_fork.ensemble.HistGradientBoostingRegressor` take cgroups quotas
...
- |Enhancement| :class:`~sklearn_fork.ensemble.HistGradientBoostingClassifier` and
  :class:`~sklearn_fork.ensemble.HistGradientBoostingRegressor` are no longer
...
- |API|: The parameter ``normalize`` of :class:`linear_model.LinearRegression`
  ...
  :class:`~sklearn_fork.pipeline.Pipeline` with ``LinearModel`` (where
...
- |Enhancement| Deprecated the default value of the `random_state=0` in
  :func:`~sklearn_fork.utils.extmath.randomized_svd`. Starting in 1.2,
...
- |Fix| Support for `np.matrix` is deprecated in
  :func:`~sklearn_fork.utils.check_array` in 1.0 and will raise a `TypeError` in
  1.2. :pr:`20165` by `Thomas Fan`_.
