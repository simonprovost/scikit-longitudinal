...
``sklearn_fork.pipeline.make_pipeline`` is public, while
`sklearn_fork.pipeline._name_estimators` is private.
``sklearn_fork.ensemble._gb.BaseEnsemble`` is private too because the whole `_gb`
module is private.
...
should for example use ``from sklearn_fork.cluster import Birch`` instead of
``from sklearn_fork.cluster.birch import Birch`` (in practice, ``birch.py`` has
