...
- |API| The `StreamHandler` was removed from `sklearn_fork.logger` to avoid
...
- |MajorFeature| Estimators can now be displayed with a rich html
  ...
  `display='diagram'` in :func:`~sklearn_fork.set_config`. The raw html can be
...
- |Enhancement| :func:`utils.validation.check_array` supports pandas'
  ...
  all :mod:`sklearn_fork.preprocessing` transformers that accept numeric inputs with
...
- |MajorFeature| Adds a HTML representation of estimators to be shown in
  ...
  `display` option in :func:`sklearn_fork.set_config`. :pr:`14180` by
  `Thomas Fan`_.
...
- |API| Estimators now have a `requires_y` tags which is False by default
  except for estimators that inherit from `~sklearn_fork.base.RegressorMixin` or
  `~sklearn_fork.base.ClassifierMixin`. This tag is used to ensure that a proper
...
- |API| The default setting `print_changed_only` has been changed from False
  ...
  `sklearn_fork.set_config(print_changed_only=False)`. Also, note that it is
