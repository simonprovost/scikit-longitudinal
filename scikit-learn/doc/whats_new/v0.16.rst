...
- :class:`sklearn_fork.neighbors.BallTree` and :class:`sklearn_fork.neighbors.KDTree`
...
- Correct ``partial_fit`` handling of ``class_prior`` for
  ...
  :class:`sklearn_fork.naive_bayes.BernoulliNB`. By `Trevor Stephens`_.
...
- Avoid skipping the first nearest neighbor in the methods ``radius_neighbors``,
  ...
  :class:`sklearn_fork.neighbors.NearestNeighbors` and family, when the query
...
- Users should now supply an explicit ``average`` parameter to
  :func:`sklearn_fork.metrics.f1_score`, :func:`sklearn_fork.metrics.fbeta_score`,
  :func:`sklearn_fork.metrics.recall_score` and
  :func:`sklearn_fork.metrics.precision_score` when performing multiclass
...
- Allow ``X=None`` in the methods ``radius_neighbors``, ``kneighbors``,
  ...
  :class:`sklearn_fork.neighbors.NearestNeighbors` and family. If set to None,
