...
from scikit_longitudinal.templates import CustomTransformerMixinEstimator, CustomClassifierMixinEstimator, \
...
class TestAllScikitLongitudinalEstimators:
    ...
    def test_classifier_filter(self):
        ...
        assert isinstance(estimators, list)
        assert all(isinstance(est, tuple) for est in estimators)
        assert all(issubclass(est[1], CustomClassifierMixinEstimator) or est[0] in [
