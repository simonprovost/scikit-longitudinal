...
class NestedTreesClassifier(CustomClassifierMixinEstimator):
    ...
    def _build_outer_tree(self, X: np.ndarray, y: np.ndarray, depth: int, outer_node_name: str,
                          tree: Optional[DecisionTreeClassifier] = None) -> 'NestedTreesClassifier.Node':
    ...
    def _find_best_tree_and_split(self, X: np.ndarray, y: np.ndarray, outer_node_name: str) -> \
        ...
        for i, group in enumerate(self.group_features):
            subset_X = X[:, group]
            tree, y_pred, gini = self._fit_inner_tree_and_calculate_gini(subset_X, y, i, outer_node_name)

            if gini < min_gini:
                min_gini = gini
                best_tree = tree
                best_split = self._create_split(X, subset_X, y, tree)

        return best_tree, best_split

    def _fit_inner_tree_and_calculate_gini(self, subset_X: np.ndarray, y: np.ndarray, group_index: int,
                                           outer_node_name: str) -> Tuple[DecisionTreeClassifier, np.ndarray, float]:
        ...
        """
        tree = DecisionTreeClassifier(max_depth=self.max_inner_depth, **self.inner_estimator_hyperparameters)
        tree.fit(subset_X, y)
        if self.save_nested_trees:
            self._save_inner_tree(tree, f'inner_tree_{outer_node_name}_group_{group_index}.png')
        y_pred = tree.predict(subset_X)
        gini = self._calculate_gini(y, y_pred)
        return tree, y_pred, gini

    def _add_children_to_node(self, node: 'NestedTreesClassifier.Node',
                              best_split: List[Tuple[np.ndarray, np.ndarray, int]], depth: int) -> None:
    ...
    def _calculate_gini(self, y: np.ndarray, y_pred: np.ndarray) -> float:
        ...
        for class_value in np.unique(y):
            ...
            if class_count > 0:
                labels, counts = np.unique(class_subset, return_counts=True)
                class_gini = 1 - sum((count / class_count) ** 2 for count in counts)
                gini += class_gini * (class_count / total_count)

        return gini

    def _create_split(self, X: np.ndarray, subset_X: np.ndarray, y: np.ndarray, tree: DecisionTreeClassifier) \
    ...
    def _save_inner_tree(self, tree: DecisionTreeClassifier, filename: str) -> None:
